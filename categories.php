<?php require_once("./includes/header.php"); ?>
    ...
    <div class="fluid-container">
      ...
      <?php 
        ...
        $post_per_page = 1;
        $status = "Published";
        $sqls = "SELECT * FROM posts WHERE post_cat_id = :id AND post_status = :status";
        $stmts = $pdo->prepare($sqls);
        $stmts->execute([
          ':id' => $_GET['id'],
          ':status' => $status
        ]);
        $post_count = $stmts->rowCount();
        if (isset($_GET['page'])) {
          $page = $_GET['page'];
          if ($page == 1) {
            $page_id = 0;
      ...
      <section id="main" class="mx-5">
        ...
        <?php 
          ...
          $sqlp = "SELECT * FROM posts WHERE post_cat_id = :post_cat_id LIMIT $page_id, $post_per_page";
          $stmtp = $pdo->prepare($sqlp);
          $stmtp->execute([':post_cat_id' => $id]);
          while ($post = $stmtp->fetch(PDO::FETCH_ASSOC)) {
            ...
            <div class="row my-4 single-post">
              ...
              <div class="col col-lg-8 col-md-12">
                ...
                <span class="posted">
                  <a href="http://localhost/blog/categories.php?id=<?php echo $post_cat_id; ?>" class="category">
                    <?php 
